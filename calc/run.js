				...
				case "symbol":
					if(variables[ast.variables[cou].data[instruccion_pointer].data]) {
						switch(variables[ast.variables[cou].data[instruccion_pointer].data].type) {
							case "function":
								run_function(variables[ast.variables[cou].data[instruccion_pointer].data], stack, start_time);
								break;
							case "symbol":
				...
				case "symbol":
					if(variables[ast.data[instruccion_pointer].data]) {
						switch(variables[ast.data[instruccion_pointer].data].type) {
							case "function":
								run_function(variables[ast.data[instruccion_pointer].data], stack, start_time);
								break;
							case "symbol":
...
function run_function(func, stack, start_time) {
							...
							case "symbol":
								if(get_variable(func.variables[cou].data[instruccion_pointer].data, scopes)) {
									switch(get_variable(func.variables[cou].data[instruccion_pointer].data).type) {
										case "function":
											run_function(get_variable(func.variables[cou].data[instruccion_pointer].data, scopes), stack, start_time);
											break;
										case "symbol":
							...
							if(!built_ins[code[code_pointer].data]) {
								if(get_variable(code[code_pointer].data, scopes)) {
									switch(get_variable(code[code_pointer].data, scopes).type) {
										case "function":
											run_function(get_variable(code[code_pointer].data, scopes), stack, start_time);
											break;
										case "symbol":
...
function built_ins_template(stack, start_time) {
	return expand({
    ...
    • Factorial: \`calc= 1 5 .. $* 1 fold\``, type: "string"});
		...
		"map, apply, apply_to"() {
			...
			var mapped = list.map(item => {
				stack.push(item);
				run_function(mapper, stack, start_time);
				return stack.pop();
			});
		...
		"reduce, fold, foldl, fold_left"() {
			...
			for(let cou = 0; cou < list.length; cou++) {
				stack.push(list[cou]);
				run_function(reducer, stack, start_time);
			}
		},
		...
		filter() {
			...
			var filtered = list.filter(item => {
				stack.push(item);
				run_function(filter, stack, start_time);
		...
		"call, run, do, apply, get"() {
			run_function(stack.pop(), stack, start_time);
		},
		"iter, iterate, iterative, loop, loopn"() {
			...
			for(let cou = 0; cou < iter_cou; cou++) {
				run_function(iterator, stack, start_time);
			}
		},
